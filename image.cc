...
bool ReadImage(const string &filename, Image *an_image) {  
  ...
  std::cout<< "Got to ReadImage\n"<<std::endl;
  FILE *input = fopen(filename.c_str(),"rb");
  std::cout<< "Got to ReadImage 2\n"<<std::endl;
  if (input == 0) {
    cout << "ReadImage: Cannot open file" << endl;
    return false;
  }
  std::cout<< "Got to ReadImage 3\n"<<std::endl;
  ...
  std::cout<< "Got to ReadImage 4\n"<<std::endl;
  // Skip comments.
  do
    fgets(line, sizeof line, input);
  while(*line == '#');
  std::cout<< "Got to ReadImage 5\n"<<std::endl;
  // Read the width and height.
  int num_columns,num_rows;
  sscanf(line,"%d %d\n", &num_columns, &num_rows);
  an_image->AllocateSpaceAndSetSize(num_rows, num_columns);
  
  std::cout<< "Got to ReadImage 5.5\n"<<std::endl;
  // Read # of gray levels.
  fgets(line, sizeof line, input);
  int levels;
  sscanf(line,"%d\n", &levels);
  an_image->SetNumberGrayLevels(levels);
  std::cout<< "Got to ReadImage 5.6\n"<<std::endl;
...
// void p1(string input_File, int threeshold, )
